<template>
  <el-card>
    ...
    <el-form :inline="true" :model="postForm" class="form-inline">
      <el-form-item label="">
        <el-input
          v-model="postForm.medicineTypeName"
          style="width: 240px"
    ...
    <el-table :data="tableData" stripe style="width: 100%" max-height="400">
      <el-table-column prop="medicineTypeName" label="分类名" />
  ...
  <el-dialog v-model="dialogVisible" title="分类编辑" width="25%">
    ...
    >
      <el-form-item label="分类ID" prop="medicineTypeId">
        <el-input v-model="updateForm.medicineTypeId" disabled />
      </el-form-item>
      <el-form-item label="分类名称" prop="typeName">
        <el-input v-model="updateForm.medicineTypeName" />
      </el-form-item>
    </el-form>
    <template #footer>
  ...
  <el-dialog v-model="dialog" title="新增分类" width="25%">
    <el-form
      ...
      style="font-weight: bold"
    >
      <el-form-item label="分类名称" prop="medicineTypeName" required>
        <el-input v-model="addForm.medicineTypeName" />
      </el-form-item>
    </el-form>
    <template #footer>
      <div class="dialog-footer">
...
import {
  getmedCategoryListByPageApi,
  postAddMedicineCategoryApi,
  putUpdMedicineCategoryApi,
  delMedicineCategoryApi,
} from "@/api/medicine";
...
const pageQuery = async () => {
  try {
    const res = await getmedCategoryListByPageApi(postForm);
...
const handleDelete = async (item) => {
  const { medicineTypeId } = item;

  try {
    const res = await delMedicineCategoryApi(medicineTypeId);
...
const handleEdit = (item) => {
  ...
  // 【注意】这里要一一赋值
  updateForm.medicineTypeId = item.medicineTypeId;
  updateForm.medicineTypeName = item.medicineTypeName;
...
const handleConfirm = async () => {
  // 发起请求
  try {
    const res = await putUpdMedicineCategoryApi(updateForm);
...
const handleAdd = async () => {
  ...
  try {
    const res = await postAddMedicineCategoryApi(addForm);
    ElMessage.success(res.msg);
